...
save_h5_path='h5/grasp_view_64_unetpp.h5'
num_classes=65   #64+1


####加载路径
##路径
test_path_x,test_path_y=load_path(sceneID_test_seen[0:1],annID[0:2],'grasp_vec_view_64')
...
def load_y(path):
    ...
    for i in path:
        ...
        img=np.int16(np.rint((np.array(img)[:, :, 0] / 255) * (num_classes - 1)))
        img_y.append(img)
