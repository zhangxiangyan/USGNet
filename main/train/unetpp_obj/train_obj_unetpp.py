...
####加载路径
##路径
path_x,path_y=load_path(sceneID_all,annID)    #只训练训练数据集，训练集又随机划分出测试集
...
test_path_x,test_path_y=path_x[n_train:n_test],path_y[n_train:n_test]
# val_path_x,val_path_y=path_x[n_test:],path_y[n_test:]


####数据生成器
train_data_loader=DataSequence_obj(train_path_x,train_path_y,batch_size,img_size,num_classes,shuffle=True,emh=True)
test_data_loader=DataSequence_obj(test_path_x,test_path_y,batch_size,img_size,num_classes,shuffle=False,emh=False)
...
####断点续训
checkpoint_save_path='./checkpoint/grasp_obj.ckpt'
