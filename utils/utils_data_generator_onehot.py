...
##生成器类
class DataSequence_onehot(keras.utils.Sequence):
    def __init__(self,x_path,y_path,batch_size,img_size,cls_num,shuffle=True,eval=False):
        self.x_path=x_path
        self.y_path=y_path
    ...
    def load_y(self,path):  #转换为独热码
        img_y=[]
        for i in path:
            img=Image.open(i)
            img=img.resize((self.img_size,self.img_size),Image.NEAREST)
            img=np.int16(np.array(img)[:,:,0]/255)
            #转换为独热码
            if self.eval:
                pass
            else:
