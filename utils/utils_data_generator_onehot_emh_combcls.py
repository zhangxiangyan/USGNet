...
##生成器类
class DataSequence_onehot_emh_combcls(keras.utils.Sequence):
    def __init__(self,x_path,y_path,batch_size,img_size,cls_num,combcls_num,shuffle=True,emh=True,eval=False):
        self.x_path=x_path
        self.y_path=y_path
        self.batch_size=batch_size
        self.img_size=img_size
        self.cls_num=cls_num
        self.combcls_num=combcls_num
    ...
    def load_x_y(self,path_x,path_y):
        ...
        for i in range(len(path_x)):
            ...
            img_out =np.array(img_out)[:, :, 0]    #读入的数值本来就是0-255

            #合并分类
            img_out[img_out!=0]=(img_out[img_out!=0]-1)//self.combcls_num+1
