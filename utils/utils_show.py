...
# """
# 根据物体类别随机选取给定数量的夹持器个数
# """
# def random_sample_obj(grasp,gripper_obj,numGrasp=20):
#     #复制数据
#     shuffled_grasp_group_array = copy.deepcopy(grasp)
#
#     #获取类别
#     obj_id=np.unique(gripper_obj)
#     obj_num=len(obj_id)
#
#     # # 复制数据2
#     a=np.arange(len(grasp))
#
#     #筛选物体类别
#     for i in range(obj_num):
#         for obj in obj_id:
#             a[50*i:50*(i+1)]=np.random.shuffle(a[gripper_obj==obj])[:50]
#
#     a=list(np.random.shuffle(a))
#     shuffled_grasp_group_array =shuffled_grasp_group_array[a]
#
#     # # np.random.shuffle(a)
#     # # return a[:numGrasp]
...
def random_sample(grasp,numGrasp=20):
    ...
    np.random.shuffle(shuffled_grasp_group_array)
    # return shuffled_grasp_group_array[:numGrasp]
    return np.random.choice(shuffled_grasp_group_array,numGrasp)


"""
按顺序给定数量的夹持器个数
"""
def sort_sample(grasp,numGrasp=20):
    sort_grasp_group_array = copy.deepcopy(grasp)
    return sort_grasp_group_array[:numGrasp]
